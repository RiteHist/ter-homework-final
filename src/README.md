...
| <a name="input_resource_names_list"></a> [resource\_names\_list](#input\_resource\_names\_list) | Список имен ресурсов, которым нужна отдельная подсеть. Каждому имени должен соответствовать отдельный CIDR в subnet\_cidr\_list. | `list(string)` | <pre>[<br/>  "web",<br/>  "db"<br/>]</pre> | no |
